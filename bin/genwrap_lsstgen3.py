...
class Gen3WrapLSST(basic_wrapper.BasicWrapper):
    ...
    def __init__(self, wclfile, debug=1):
        ...
        if 'wrapper' in self.inputwcl:
            ...
            if 'job_repo_dir' in self.inputwcl['wrapper']:
                ...
                if 'butler_template' in self.inputwcl['wrapper']:
                    ...
                    with open(btfile) as infh:
                        with open(os.path.join(jrdir, 'butler.yaml'), "w") as outfh:
                            for line in infh:
                                newline = repfunc.replace_vars_single(line, self.inputwcl,
                                                                      {intgdefs.REPLACE_VARS: True,
                                                                      'expand': True, 'keepvars': False})
                                outfh.write(newline)
                else:
...
def main():
    """Entry point.
    """
    parser = argparse.ArgumentParser(description='Generic wrapper for LSST using Gen3 Middleware')
    parser.add_argument('inputwcl', nargs=1, action='store')
    args = parser.parse_args(sys.argv[1:])

    bwrap = Gen3WrapLSST(args.inputwcl[0])
    bwrap.run_wrapper()
